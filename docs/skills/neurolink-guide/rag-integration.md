...
async function ragPipeline(filePaths: string[], query: string) {
  ...
  const chunker = createChunker('markdown', { chunkSize: 512 });
  const allChunks = [];
...
NeuroLink supports 22 vector store adapters:
...
- [Memory](memory-conversations.md) - Conversation history
- [Tools](tools-mcp.md) - MCP integration
- [Advanced features](advanced-features.md) - Workflows
