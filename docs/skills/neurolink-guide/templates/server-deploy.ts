...
function showRoutes(): void {
  ...
  const routes = [
    ...
    { method: "POST", path: "/api/stream", description: "Streaming generation" },
  ...
  for (const route of routes) {
    console.log(`${route.method.padEnd(7)} ${route.path.padEnd(30)} ${route.description}`);
  }
}
...
function showCLICommands(): void {
  ...
  const commands = [
    { command: "neurolink serve", description: "Start default server (port 3000)" },
    ...
    { command: "neurolink serve --rate-limit", description: "Enable rate limiting" },
    { command: "neurolink serve --swagger", description: "Enable Swagger docs" },
