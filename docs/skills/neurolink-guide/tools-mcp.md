...
emitter.on('tool:end', (event) => {
  console.log(`Tool ${event.toolName}: ${event.success ? 'success' : 'failed'}`);
  console.log(`Duration: ${event.responseTime}ms`);
});

emitter.on('externalMCP:serverConnected', (event) => {
  console.log(`MCP server ${event.serverId} connected with ${event.toolCount} tools`);
});
...
- [Memory](memory-conversations.md) - Conversation memory
- [Advanced features](advanced-features.md) - HITL, workflows
